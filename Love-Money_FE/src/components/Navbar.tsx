...
import Rulebook from "./model/Rulebook";

const Navbar: React.FC = () => {
  const [isRulebookOpen, setIsRulebookOpen] = useState(false);

  const handleRulebookClick = () => setIsRulebookOpen(true);
  ...
  return (
    <>
      <header className="fixed left-0 top-0 z-50 w-screen">
        ...
        <div className="relative flex w-full items-center justify-between px-4 py-4 text-white">
          ...
          <nav style={{ fontFamily: "DungGeunMo" }} className="flex space-x-10">
            <button className="hover:scale-105" onClick={handleRulebookClick}>
              룰북
            </button>
      ...
      <Rulebook isOpen={isRulebookOpen} onClose={handleCloseRulebook} />
    </>
  );
};
